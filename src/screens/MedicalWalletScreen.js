...
export default function MedicalWalletScreen({ navigation, user }) {
  const { width } = useWindowDimensions();
  
  // Use user-specific data when user is logged in
  ...
  const updatePetImage = async (imageUri) => {
    try {
      const petDocRef = doc(db, "users", userId, "profile", "pet_profile");
  ...
  if (currentView === 'vaccinations') {
    return <VaccinationList onBack={goBack} navigate={navigate} user={user} />;
  }
  if (currentView === 'prescriptions') {
    return <PrescriptionList onBack={goBack} navigate={navigate} user={user} />;
  }
  if (currentView === 'microchip') {
    return <MicrochipDetails onBack={goBack} navigate={navigate} user={user} />;
  }
  if (currentView === 'vetvisits') {
    return <VetVisitList onBack={goBack} navigate={navigate} user={user} />;
  }
  if (currentView === 'add-vaccination') {
    return <AddVaccination onBack={goBack} navigate={navigate} user={user} />;
  }
  if (currentView === 'add-prescription') {
    return <AddPrescription onBack={goBack} navigate={navigate} user={user} />;
  }
  if (currentView === 'add-vet-visit') {
    return <AddVetVisit onBack={goBack} navigate={navigate} user={user} />;
  }
  if (currentView === 'edit-vaccination') {
    return <EditVaccination onBack={goBack} navigate={navigate} params={navigationParams} user={user} />;
  }
  if (currentView === 'edit-prescription') {
    return <EditPrescription onBack={goBack} navigate={navigate} params={navigationParams} user={user} />;
  }
  if (currentView === 'edit-vet-visit') {
    return <EditVetVisit onBack={goBack} navigate={navigate} params={navigationParams} user={user} />;
  }
  if (currentView === 'record-details') {
    return <RecordDetails onBack={goBack} navigate={navigate} params={navigationParams} user={user} />;
  }
